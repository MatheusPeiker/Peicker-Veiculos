...
const Home: React.FC = () => {
  const featuredCars = CARS_DATA.filter(car => car.isFeatured);
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20">
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl py-20">
          ...
          <div className="flex flex-col sm:flex-row gap-6 justify-center animate-fade-in-up delay-300">
            <Link to="/inventory" className="bg-primary text-black font-black py-6 px-16 rounded-2xl text-sm hover:scale-105 transition-all flex items-center justify-center gap-3 uppercase shadow-2xl shadow-primary/30">
      ...
      <section className="py-32 px-4 bg-white dark:bg-background-dark">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row items-end justify-between mb-20">
            ...
            <Link to="/inventory" className="group flex items-center gap-3 text-xs font-black uppercase tracking-widest border-b-2 border-primary pb-2 hover:text-primary transition-all mt-8 md:mt-0">
            ...
            {featuredCars.map(car => (
              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden">
                  <img src={car.images[0]} alt={car.model} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
                ...
                <div className="p-10">
                  <p className="text-[10px] font-black text-primary uppercase tracking-[0.3em] mb-2">{car.brand}</p>
                  <h3 className="text-2xl font-bold mb-6 dark:text-white group-hover:text-primary transition-colors leading-none tracking-tight">{car.model}</h3>
                  <div className="flex justify-between items-center mb-8 py-5 border-y border-slate-200 dark:border-white/5">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="text-[10px] font-black text-slate-500 uppercase">{car.km}</span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="material-icons-round text-primary text-lg mb-1">local_gas_station</span>
                      <span className="text-[10px] font-black text-slate-500 uppercase">{car.fuel}</span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="material-icons-round text-primary text-lg mb-1">calendar_today</span>
                      <span className="text-[10px] font-black text-slate-500 uppercase">{car.year}</span>
                    </div>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-3xl font-black text-slate-900 dark:text-white italic tracking-tighter">R$ {car.price.toLocaleString('pt-BR')}</span>
